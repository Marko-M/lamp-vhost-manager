...
GRANT USAGE ON * . * TO '$MYSQLU'@'%' IDENTIFIED BY '$MYSQLP' WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;
CREATE DATABASE IF NOT EXISTS \`$MYSQLN\`;
GRANT ALL PRIVILEGES ON \`$MYSQLN\`. * TO '$MYSQLU'@'%';
QUERY_INPUT
    else
